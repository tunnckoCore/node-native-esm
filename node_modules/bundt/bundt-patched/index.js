...
// Parsed config
const fields = {
	main: pkg.main || 'dist/index.cjs.js',
	unpkg: pkg.unpkg || 'dist/index.unpkg.js',
	module: pkg.module || 'src/index.js',
	browser: pkg.browser || 'dist/index.umd.js',
};
...
function safePackageName(str) {
	return snakeToCamel(str.replace('@', '').replace('/', '.'));
}

const name = pkg['umd:name'] || pkg.name;
const mount = `.${safePackageName(name)}`
